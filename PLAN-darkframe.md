...
**Tasks**:
- [ ] Submit to the Chrome Web Store (one-time $5 developer fee, absorbed by the project) — AC: listing is live and installable via a public Chrome Web Store URL. **Fully prepared, not yet submitted**: `store/chrome/listing.md` (all copy, permission justifications, privacy disclosures), `store/chrome/screenshots/` + `store/chrome/promo/` (generated, verified against the real running extension), and `scripts/package-chrome.mjs` (produces the upload zip) all exist — see RELEASING.md for the remaining account-creation-and-click-submit steps, which require your action (payment, account access) and can't be done from this environment.
- [x] Resolve the Safari distribution Open Question (see below) with an explicit decision, then execute it: either (a) maintainer funds Apple Developer Program ($99/yr) and ships a notarized direct-distribution build, or (b) ship build-from-source only and document that choice publicly — **Decided**: (a), funding it. Apple Developer Program enrollment is submitted and pending identity verification (not yet approved as of this writing). `store/safari/app-store-connect.md` is fully prepared for the moment it clears — see RELEASING.md.
- [ ] Run the full Playwright E2E matrix (Chromium + WebKit engines) as a required, green CI gate on the release tag — AC: `pnpm test:e2e` passes in CI for the tagged release commit. **Not done** — CI (`.github/workflows/ci.yml`) runs the Vitest unit suite only; the real-browser E2E script (`tests/e2e/verify-extension.mjs`) is documented as run-manually-only (see its own header comment) and has not been wired into CI. Remains a real gap, not silently dropped.
- [x] Write a public privacy policy covering the `host_permissions: *://*/*` grant (required by both Chrome Web Store and Apple review) — AC met: `PRIVACY.md` exists, explicitly states no telemetry/analytics are collected, and is referenced by both `store/chrome/listing.md` and `store/safari/app-store-connect.md`.
**Exit criteria**: A first-time user can install Darkframe from a public listing (Chrome Web Store, and whichever Safari path was chosen) without contacting the maintainer, on a machine that has never had the extension before. **Not yet met** — both listings are submission-ready but not yet submitted/live; this is the one remaining gate and it requires your action, not more engineering (see RELEASING.md).
...
- [x] **Safari distribution funding decision**: does the project commit to an ongoing $99/year personal Apple Developer Program cost to ship a signed/notarized direct-distribution build for non-technical users, or does v1 ship build-from-source-only on Safari and revisit funded distribution later? — **Resolved**: fund it. Enrollment submitted, pending Apple's identity verification as of this writing. Mac App Store submission (see RELEASING.md) is ready to execute the moment it's approved; build-from-source remains documented and available in the meantime regardless.
...
- [x] **Project name/branding**: "Umbra" was the original working name and was **not** checked against existing Chrome Web Store collisions before this question was written. **Resolved during Phase 6 prep**: that check found a direct, active, same-category collision ("Umbra Dark Mode," ~10k users) — the project was renamed to "Darkframe" (collision-checked clear) before any store listing was created. See Appendix C for the full account.
...
"Umbra" (Latin: shadow) was the original working project name, chosen for its thematic fit and because it did not collide with any existing folder in `~/Documents/Personal`. It was never checked against Chrome Web Store collisions before this section was written.

**Renamed to "Darkframe" during shipping prep (2026-07-11)**: that check, done as part of Chrome Web Store/Safari shipping readiness work, found an existing, active Chrome extension called "Umbra Dark Mode" (~10,000 users, same category — dark mode with a per-site whitelist, last updated March 2025) plus an unrelated "Umbra Dark" Firefox theme. Renaming before any Umbra store listing existed was judged far cheaper than renaming after real installs/reviews existed, so the project was renamed to "Darkframe" — collision-checked clear (no Chrome Web Store, Firefox, or software-trademark hits found) — across the npm scope, extension manifest, internal message/storage-key prefixes, the injected CSS layer name, and the Safari Xcode project (regenerated fresh via `safari-web-extension-converter` rather than hand-edited, to avoid corrupting `project.pbxproj` path references). The GitHub repository itself was deliberately left at `github.com/AshwinSathian/umbra` — renaming an existing repo is a separate, larger decision not bundled into this rename.
